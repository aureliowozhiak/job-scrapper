    ...
    def _make_request_with_retry(self, url: str) -> requests.Response:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.debug(f"Successfully fetched {url} (content length: {len(response.text)} chars)")
                return response
        ...
        for site, data in self.urls.items():
            ...
            try:
                ...
                if html_response.status_code == 200:
                    ...
                    # Use response.content and let requests handle decompression
                    with open(file_name_path, "w", encoding='utf-8') as f:
                        f.write(html_response.text)
                    
                    # Verify file was written correctly
                    if len(html_response.text) < 100:
                        logger.warning(f"Suspiciously small response ({len(html_response.text)} chars) from {site}")
